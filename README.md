...
Check how `static_paths` are defined:
...
For example, set `src={Vite.path("js/app.js")}` so `Vite` will serve it.
...
At compile time, it will bundle teh files with the correct fingerprinted name.
